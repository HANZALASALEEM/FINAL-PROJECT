...
const StudentLoginScreen = () => {
  ...
  const [email, setEmail] = useState('hanzala.4298@suffah.edu.pk');
  ...
  const signIn = async () => {
    try {
      ...
      if (student.length > 0) {
        ...
        navigation.navigate('StudentBottomNavigator', {data: student[0]});
      } else {
        Alert.alert(
          'Try Again',
          'Student not found. If your email and password is correct Try again with better internet connection.',
        );
      }
    } catch (error) {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.bannerContainer}>
        ...
        <Text style={styles.bannerContainerTitle}>
          SIGN IN WITH PROVIDED STUDENT EMAIL
        </Text>
      </View>
      ...
      <View>
        <TitleAndInput
          ...
          placeholder={'name.rollNo@suffah.edu.pk'}
          title={'EMAIL'}
